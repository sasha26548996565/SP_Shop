            ...
            <div class="basis-auto xl:basis-3/4">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div class="flex items-center gap-4">
                        {{-- active have class: pointer-events-none text-pink ; no active have class hover:text-pink text-white --}}
                        <div class="flex items-center gap-2">
                            <a href="{{ filter_url($category, ['view' => 'grid']) }}"
                                class="inline-flex items-center justify-center w-10 h-10 rounded-md bg-card
                                @if (is_catalog_view('grid')) text-pink pointer-events-none
                                @else
                                    hover:text-pink text-white @endif">
                            ...
                            <a href="{{ filter_url($category, ['view' => 'list']) }}"
                                class="inline-flex items-center justify-center w-10 h-10 rounded-md bg-card
                                @if (is_catalog_view('list')) pointer-events-none text-pink
                                @else
                                    hover:text-pink text-white @endif">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                    viewBox="0 0 52 52">
                    ...
                    <div x-data="{ sort: '{{ filter_url($category, ['sort' => request('sort')]) }}' }" class="flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        <select name="sort" x-model="sort" x-on:change="window.location = sort"
                            ...
                            <option value="{{ filter_url($category, ['sort' => 'price']) }}" class="text-dark">
                ...
                <!-- Products list -->
                <div
                    class="products grid grid-cols-1
                        @if (is_catalog_view('grid')) sm:grid-cols-2 xl:grid-cols-3 gap-x-6 2xl:gap-x-8 gap-y-8 lg:gap-y-10 2xl:gap-y-12
                        @else 
                            gap-y-8 @endif">
